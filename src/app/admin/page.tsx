...
type FormData = { team_name: string; player_name: string; pubg_account_id: string };
const EMPTY: FormData = { team_name: "", player_name: "", pubg_account_id: "" };
...
export default function AdminPage() {
  ...
  const startEdit = (p: SteamPlayer) => {
    ...
    setEditForm({
      ...
      pubg_account_id: p.pubg_player_id ?? "",
    });
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="text-left text-white/40 border-b border-white/10">
                ...
                <th className="pb-3 pr-4">PUBG 계정 ID</th>
                ...
                editId === p.id ? (
                  <tr key={p.id} className="border-b border-white/5 bg-white/5">
                    <td className="py-2 pr-3">
                      <input value={editForm.team_name} onChange={(e) => setEditForm({ ...editForm, team_name: e.target.value })} className="input w-28" />
                    </td>
                    <td className="py-2 pr-3">
                      <input value={editForm.player_name} onChange={(e) => setEditForm({ ...editForm, player_name: e.target.value })} className="input w-28" />
                    </td>
                    <td className="py-2 pr-3">
                      <input value={editForm.pubg_account_id} onChange={(e) => setEditForm({ ...editForm, pubg_account_id: e.target.value })} className="input w-48 font-mono text-xs" />
                    </td>
                    <td className="py-2 pr-3">
                      <select value={editActive ? "1" : "0"} onChange={(e) => setEditActive(e.target.value === "1")} className="input w-20">
                        <option value="1">활성</option>
                        <option value="0">비활성</option>
                      </select>
                    </td>
                    <td className="py-2 flex gap-2">
                      <button onClick={() => handleSave(p.id)} className="text-pubg-gold hover:text-yellow-300 text-xs">저장</button>
                      <button onClick={() => setEditId(null)} className="text-white/40 hover:text-white text-xs">취소</button>
                    </td>
                  </tr>
                ) : (
                  <tr key={p.id} className={`border-b border-white/5 ${!p.is_active ? "opacity-40" : ""}`}>
                    <td className="py-3 pr-4 text-white font-medium">{p.team_name}</td>
                    <td className="py-3 pr-4 text-white">{p.player_name}</td>
                    <td className="py-3 pr-4 text-white/50 font-mono text-xs">{p.pubg_player_id ?? "—"}</td>
                    <td className="py-3 pr-4">
                      <span className={`text-xs px-2 py-0.5 rounded-full ${p.is_active ? "bg-blue-900/50 text-blue-400" : "bg-white/10 text-white/30"}`}>
                        {p.is_active ? "활성" : "비활성"}
                      </span>
                    </td>
                    <td className="py-3 flex gap-3">
                      <button onClick={() => startEdit(p)} className="text-white/50 hover:text-white text-xs transition-colors">수정</button>
                      <button onClick={() => handleDelete(p.id, p.player_name)} className="text-red-500/60 hover:text-red-400 text-xs transition-colors">삭제</button>
                    </td>
                  </tr>
                )
              )}
